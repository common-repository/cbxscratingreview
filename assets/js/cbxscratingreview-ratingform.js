	...
	jQuery(document).ready(function ($) {
		...
		$('.cbxscratingreview_mainwrap').each(function (index, element) {
			...
			//console.log(cbxscratingreview_ratingform.half_rating);

			/*$wrapper.find('.cbxscratingreview-form-guest-note a').on('click', function(e) {
				e.preventDefault();

				$wrapper.find('.cbxscratingreview-form-guest-login-wrapper').toggle();
			...
			$form.submit(function (e) {
				...
				let $post_id = Number($form.data('postid'));
				...
				if ($formvalidator.valid() && $busy === 0) {
					...
					$.ajax({
						...
						data    : $form.serialize() + '&action=cbxscratingreview_review_rating_frontend_submit' + '&security=' + cbxscratingreview_ratingform.nonce,// our data object
						success : function (data) {						
							let $global_msg = $form.prev('.cbxscratingreview_global_msg');
							...
							if (data.ok_to_process === 1) {
								...
								if (!$.isEmptyObject(data.success.responsedata)) {
									...
									if (data.success.responsedata.review_html !== undefined && data.success.responsedata.review_html !== '') {
										...
										let $review_list_html = $('.cbxscratingreview_review_list_items');
										if ($review_list_html.length > 0) {
											if ($review_list_html.find('.cbxscratingreview_review_list_item_notfound').length) {
												$review_list_html.find('.cbxscratingreview_review_list_item_notfound').remove();
                                                $('#cbxscratingreview_search_wrapper_'+$post_id).show();
											}
											$review_list_html.prepend(data.success.responsedata.review_html);
											cbxscratingreview_readonlyrating_process($);


											let $scroll_to = $global_msg.offset().top - 50;

											$('html, body').animate({
												scrollTop: $scroll_to
											}, 1000);

											CBXSCRatingReviewEvents_do_action('cbxscratingreview_review_entry_success', $);
										}
									}
								}
							} else {
		...
		}); //end each .cbxscratingreview_mainwrap
	});